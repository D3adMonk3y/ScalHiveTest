...
public class User2{
    ...
    public User2(String userName, boolean paid) {
        id = userCount++;

        //checks if the name of new user is satisfies the conditions that it is not empty and consists only with "[a-z][A-Z][0-9]-._"
        while(userName.equals("") || !namePattern.matcher(userName).matches()){
            TextInputDialog dialog = new TextInputDialog();
            dialog.setTitle("Name Input Dialog");
            dialog.setHeaderText("User name can only be english letters numbers and '.', '_', '-'\n" +
                    "and can't be empty.");
            dialog.setContentText("Please enter your new name:");

// Traditional way to get the response value.
            userName = String.valueOf(dialog.showAndWait());
        }
